...
- `cmake -S Desktop/Windows -B Desktop/Windows/build -G "Visual Studio 17 2022" -A x64 -DCMAKE_TLS_VERIFY=ON && cmake --build Desktop/Windows/build --config Release` (Visual Studio C++ workload; use the installed version's generator name: `"Visual Studio 17 2022"` needs CMake 3.21+, `"Visual Studio 16 2019"` works with CMake 3.20+, `"Visual Studio 18 2026"` needs CMake 4.2+; the WebView2 SDK is downloaded from NuGet at configure time, pin with `-DSTASH_WEBVIEW2_VERSION`). `build_plugin.ps1` wraps it and takes `-Generator` and `-CaBundle`. Pin the generator explicitly: with Ninja on PATH, CMake selects it by default and `-A x64` is then rejected. Always pass `-DCMAKE_TLS_VERIFY=ON`: CMake before 3.31 defaults download verification off, and this fetch is a binary SDK that links into the payments host. If verification then fails on a CMake without a CA bundle (e.g. WinLibs), add `-DCMAKE_TLS_CAINFO=<path to ca-bundle.crt>` rather than turning verification off.
