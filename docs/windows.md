...
Event types: `paymentSuccess` (order or empty), `paymentFailure`, `dialogDismissed`, `optInResponse`, `pageLoaded` (ms), `networkError`, `externalPayment` (themed URL), `purchaseProcessing`, `processingCompleted`; diagnostics `navigation` (origin `scheme://host`, never the URL: it carries the signed token and wrappers log these), `navigationBlocked` (`{"url": origin, "reason"}`), `webProcessCrashed`, `error` (a code, never a URL).
